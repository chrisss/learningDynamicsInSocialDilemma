    ...
    for j = 1:3
        for i = 1:3
                ...
                if (P(i,j) - 0.001) >= (-0.0000001)
                    if (summe - Q(i,j))/8 <= Q(i,j) %if neighbours do cooperate more, cooperation will decrease! 
                ...
                if (P(i,j) + 0.001) <= 1.0000001 
                    if (summe - Q(i,j))/8 >= Q(i,j) %if neighbours do not cooperate, cooperation will increase!
...
for n = 1:9
    ...
    ylabel('Probability for different players');
